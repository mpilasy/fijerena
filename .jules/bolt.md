...
## 2025-05-18 - Avoid String Allocation During List Sorting and Extension Matching
**Learning:** During in-memory list sorting, using `.sortedBy { it.lowercase() }` will allocate a new String object for every single comparison step, causing massive GC overhead for large collections. Also, extracting and lowercasing extensions (`substringAfterLast().lowercase()`) combined with `setOf` checking generates redundant string copies and iterators in hot paths like recursive directory scanning.
**Action:** Replace `.sortedBy { it.lowercase() }` with `.sortedWith(compareBy(String.CASE_INSENSITIVE_ORDER) { it })`. Replace substring operations with index-based array iteration and `regionMatches(..., ignoreCase = true)` to avoid any object allocations.
