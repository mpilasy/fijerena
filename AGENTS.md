...
- **Database Maintenance & Compaction:** When providers are deleted, `ProviderRepository.deleteProvider` cascades through all catalog tables in `xtream_v2.db` (`xtream_streams`, `xtream_series`, `xtream_episodes`, `xtream_categories`, `favorite_state`, `xtream_epg_cache`), clears orphaned SharedPreferences (`provider_creds_*`, `media_cache_*`, `xtream_cache_*`), and runs `VACUUM` + `PRAGMA wal_checkpoint(TRUNCATE)`. A background self-healing sweep (`pruneOrphanedCatalogData`) runs on app launch and during `EpgSyncWorker` to clean up past orphaned rows. The "Shrink Database" button in Settings forces compaction on demand. A circuit breaker prevents deletions if the valid provider list is empty.
