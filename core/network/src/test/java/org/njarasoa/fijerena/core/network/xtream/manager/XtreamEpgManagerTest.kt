...
class XtreamEpgManagerTest {
    ...
    @Test
    fun `getEpgForStreams returns correct results`() = runBlocking {
        val apiService = mockk<XtreamApiService>()
        val manager = createManager(apiService)

        coEvery { apiService.getEpgForStream(1) } returns EpgResponse(
            listOf(EpgProgram(id = "p1", title = "Program 1", start = "100", end = "200"))
        )
        coEvery { apiService.getEpgForStream(2) } returns EpgResponse(
    ...
    @Test
    fun `getEpgForStreams continues on individual stream failure`() = runBlocking {
        val apiService = mockk<XtreamApiService>()
        val manager = createManager(apiService)
    ...
    @Test
    fun `getEpgForStreams with empty list returns empty map`() = runBlocking {
        val apiService = mockk<XtreamApiService>()
        val manager = createManager(apiService)
    ...
    @Test
    fun `getEpgForStreams resolves API service once`() = runBlocking {
        val apiService = mockk<XtreamApiService>()
        val sharedPrefs = mockk<SharedPreferences>(relaxed = true)
        val providerSettings = mockk<ProviderSettings>()
        val sessionManager = mockk<XtreamSessionManager>()
    ...
    @Test
    fun `benchmark getEpgForStreams with 50 streams`() = runBlocking {
        val apiService = mockk<XtreamApiService>()
        val manager = createManager(apiService)
