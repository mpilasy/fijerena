...
class XtreamRepository(
    ...
    private val statsManager = XtreamStatsManager(
        database = database,
    ...
    private val sessionManager = XtreamSessionManager(
        ...
        onClearCache = { statsManager.clearCache() }
    )
    ...
    private val userDataManager = XtreamUserDataManager(
        sharedPreferences = cache,
        providerSettings = providerSettings
    )

    private val epgManager = XtreamEpgManager(
        sessionManager = sessionManager,
        sharedPreferences = cache,
        providerSettings = providerSettings
    )
    ...
    companion object {
        // Kept for backward compatibility if accessed statically
    }
    ...
    fun buildStreamUrl(streamId: Int, contentType: String = "LIVE_TV", extension: String? = null): Result<String> =
    ...
    fun clearEpgCache(streamId: Int) {
        epgManager.clearEpgCache(streamId)
    }

    fun clearAllEpgCache() {
        epgManager.clearAllEpgCache()
    }
    ...
    fun clearCacheForContentType(contentType: String) {
        statsManager.clearCacheForContentType(contentType)
    }

    fun clearCategoriesCache(contentType: String) {
        statsManager.clearCategoriesCache(contentType)
    }

    fun clearStreamsCache(categoryId: String) {
        statsManager.clearStreamsCache(categoryId)
    }
