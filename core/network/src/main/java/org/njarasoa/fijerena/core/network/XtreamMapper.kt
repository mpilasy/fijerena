...
object XtreamMapper {
    ...
    fun XtreamCategory.toDomain(): MediaCategory = MediaCategory(
        id = categoryId,
        name = categoryName,
        parentId = if (parentId != 0) parentId.toString() else null
    )

    fun XtreamStream.toDomain(mediaType: MediaType): MediaItem = MediaItem(
    ...
    fun XtreamSeries.toDomain(): MediaItem = MediaItem(
    ...
    fun SeriesInfo.toDomain(seriesId: String): SeriesDetail = SeriesDetail(
        id = seriesId,
        name = info?.name ?: "",
        metadata = MediaMetadata(
            plot = info?.plot,
        ...
        episodes = episodes.mapValues { (_, episodeList) ->
            episodeList.map { it.toDomain() }
        }
    )

    fun Season.toDomain(): SeasonInfo = SeasonInfo(
    ...
    fun Episode.toDomain(): EpisodeItem = EpisodeItem(
        id = id,
    ...
    fun VodInfo.toDomain(movieId: String): MovieDetail = MovieDetail(
        id = movieId,
    ...
    fun FavoriteStream.toDomain(mediaType: MediaType): MediaItem = MediaItem(
        id = streamId.toString(),
        name = streamName,
        mediaType = mediaType,
        categoryId = categoryId
