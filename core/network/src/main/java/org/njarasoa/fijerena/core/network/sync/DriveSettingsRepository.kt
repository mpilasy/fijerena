 ...
 */
class DriveSettingsRepository(private val context: Context) {
     ...
     */
    suspend fun downloadSettings(credential: GoogleAccountCredential): SyncedSettings? = withContext(Dispatchers.IO) {
        try {
     ...
     */
    suspend fun deleteSettings(credential: GoogleAccountCredential): Boolean = withContext(Dispatchers.IO) {
        try {
            val driveService = getDriveService(credential)
            val fileId = findSettingsFileId(driveService)
            if (fileId != null) {
     ...
     */
    suspend fun hasSettingsFile(credential: GoogleAccountCredential): Boolean = withContext(Dispatchers.IO) {
        try {
            val driveService = getDriveService(credential)
